...
- **Breaking:** `clear(...args, options?)` and `clearSync(...args, options?)`
  ...
  Passing a trailing `{ force: true }` options object skips the walk and
  removes the resolved path wholesale, folder and all; the folder is
  re-created on the next write. Both still return `void`, and a missing
  path is a no-op. `invalidate`/`invalidateSync` are unchanged — use them
  (or `force`) for the old unconditional single-entry delete. See
